...
describe("EditorToolbar", () => {
    ...
    it("calls onTitleChange when the title input changes", () => {
        render(<EditorToolbar {...BASE} />);
        fireEvent.change(screen.getByLabelText("editor.titleAriaLabel"), {
    ...
    it("opens the delete confirmation dialog when the delete button is clicked", () => {
        ...
        fireEvent.click(screen.getByLabelText("editor.deleteNote"));
    ...
    it("hides the save status indicator when status is idle", () => {
        ...
        expect(screen.queryByText("editor.saving")).not.toBeInTheDocument();
    ...
    it("does not render back button when onBack is not provided", () => {
        render(<EditorToolbar {...BASE} />);
        expect(screen.queryByLabelText("editor.back")).not.toBeInTheDocument();
    });

    it("renders back button when onBack is provided", () => {
        render(<EditorToolbar {...BASE} onBack={jest.fn()} />);
        expect(screen.getByLabelText("editor.back")).toBeInTheDocument();
    });

    it("calls onBack when back button is clicked", () => {
        const onBack = jest.fn();
        render(<EditorToolbar {...BASE} onBack={onBack} />);
        fireEvent.click(screen.getByLabelText("editor.back"));
        expect(onBack).toHaveBeenCalledTimes(1);
    });

    it("does not render Toggle AI Tools button when onToggleAI is not provided", () => {
        render(<EditorToolbar {...BASE} />);
        expect(screen.queryByLabelText("editor.toggleAI")).not.toBeInTheDocument();
    });

    it("renders Toggle AI Tools button when onToggleAI is provided", () => {
        render(<EditorToolbar {...BASE} onToggleAI={jest.fn()} />);
        expect(screen.getByLabelText("editor.toggleAI")).toBeInTheDocument();
    });
    ...
    it("calls onToggleAI when the Toggle AI Tools button is clicked", () => {
        ...
        fireEvent.click(screen.getByLabelText("editor.toggleAI"));
